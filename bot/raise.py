...
def resume_raise():
    try:
        ...
        success_button = wait.until(
            EC.element_to_be_clickable((By.XPATH, '//button[contains(., "Поднимать автоматически")]')))
...
def main():
    ...
    if (resume_raise() == "success"):
        ...
        print(
            "Something went wrong, hh.ru HTML might have been updated, so please update button selectors in the RAISE.PY code.\n")
    driver.close()
    driver.quit()
